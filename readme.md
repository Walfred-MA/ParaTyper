...
Gene chains cannot cross a gap between consecutive target alignment blocks
larger than **1.5 × the reference gene span**. A gap equal to that limit is
allowed; a larger gap starts a separate locus. Padding both sides of a local
search window does not double this chaining limit. The reference
